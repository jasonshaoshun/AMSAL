...
if __name__ == "__main__":
    ...
    df = df.sort_values(by=["debiasing", "seed"])
    ...
    SAL_mean_squared_error_after = df[df['debiasing'] == "SAL"]['mean_squared_error_after'].mean()
    SAL_mean_absolute_error_after = df[df['debiasing'] == "SAL"]['mean_absolute_error_after'].mean()
    SAL_MSE_age_after = df[df['debiasing'] == "SAL"]['MSE_age_after'].mean()
    SAL_MAE_age_after = df[df['debiasing'] == "SAL"]['MAE_age_after'].mean()
    SAL_MSE_gender_after = df[df['debiasing'] == "SAL"]['MSE_gender_after'].mean()
    SAL_MAE_gender_after = df[df['debiasing'] == "SAL"]['MAE_gender_after'].mean()

    INLP_mean_squared_error_after = df[df['debiasing'] == "INLP"]['mean_squared_error_after'].mean()
    INLP_mean_absolute_error_after = df[df['debiasing'] == "INLP"]['mean_absolute_error_after'].mean()
    INLP_MSE_age_after = df[df['debiasing'] == "INLP"]['MSE_age_after'].mean()
    INLP_MAE_age_after = df[df['debiasing'] == "INLP"]['MAE_age_after'].mean()
    INLP_MSE_gender_after = df[df['debiasing'] == "INLP"]['MSE_gender_after'].mean()
    INLP_MAE_gender_after = df[df['debiasing'] == "INLP"]['MAE_gender_after'].mean()

    df_SAL_mean = pd.DataFrame({'pretty_model_name': r"\, + \textsc{WL} + \textsc{SAL}",
                        'mean_squared_error_before': df.iloc[0]['mean_squared_error_before'],
                        'mean_squared_error_after': SAL_mean_squared_error_after,
                        'mean_absolute_error_before': df.iloc[0]['mean_absolute_error_before'],
                        'mean_absolute_error_after': SAL_mean_absolute_error_after,
                        'MSE_age_before': df.iloc[0]['MSE_age_before'],
                        'MSE_age_after': SAL_MSE_age_after,
                        'MAE_age_before': df.iloc[0]['MAE_age_before'],
                        'MAE_age_after': SAL_MAE_age_after,
                        'MSE_gender_before': df.iloc[0]['MSE_gender_before'],
                        'MSE_gender_after': SAL_MSE_gender_after,
                        'MAE_gender_before': df.iloc[0]['MAE_gender_before'],
                        'MAE_gender_after': SAL_MAE_gender_after},
                        index=[0])

    df_SAL_mean["pretty_mean_squared_error"] = df_SAL_mean.apply(lambda row: _pretty_score(row, 'mean_squared_error_before', 'mean_squared_error_after'), axis=1)
    df_SAL_mean["pretty_mean_absolute_error"] = df_SAL_mean.apply(lambda row: _pretty_score(row, 'mean_absolute_error_before', 'mean_absolute_error_after'), axis=1)
    df_SAL_mean["pretty_MSE-age_value"] = df_SAL_mean.apply(lambda row: _pretty_score(row, 'MSE_age_before', 'MSE_age_after'), axis=1)
    df_SAL_mean["pretty_MAE-age_value"] = df_SAL_mean.apply(lambda row: _pretty_score(row, 'MAE_age_before', 'MAE_age_after'), axis=1)
    df_SAL_mean["pretty_MSE-gender_value"] = df_SAL_mean.apply(lambda row: _pretty_score(row, 'MSE_gender_before', 'MSE_gender_after'), axis=1)
    df_SAL_mean["pretty_MAE-gender_value"] = df_SAL_mean.apply(lambda row: _pretty_score(row, 'MAE_gender_before', 'MAE_gender_after'), axis=1)

    df_INLP_mean = pd.DataFrame({'pretty_model_name': r"\, + \textsc{WL} + \textsc{INLP}",
                        'mean_squared_error_before': df.iloc[0]['mean_squared_error_before'],
                        'mean_squared_error_after': INLP_mean_squared_error_after,
                        'mean_absolute_error_before': df.iloc[0]['mean_absolute_error_before'],
                        'mean_absolute_error_after': INLP_mean_absolute_error_after,
                        'MSE_age_before': df.iloc[0]['MSE_age_before'],
                        'MSE_age_after': INLP_MSE_age_after,
                        'MAE_age_before': df.iloc[0]['MAE_age_before'],
                        'MAE_age_after': INLP_MAE_age_after,
                        'MSE_gender_before': df.iloc[0]['MSE_gender_before'],
                        'MSE_gender_after': INLP_MSE_gender_after,
                        'MAE_gender_before': df.iloc[0]['MAE_gender_before'],
                        'MAE_gender_after': INLP_MAE_gender_after},
                        index=[0])

    df_INLP_mean["pretty_mean_squared_error"] = df_INLP_mean.apply(lambda row: _pretty_score(row, 'mean_squared_error_before', 'mean_squared_error_after'), axis=1)
    df_INLP_mean["pretty_mean_absolute_error"] = df_INLP_mean.apply(lambda row: _pretty_score(row, 'mean_absolute_error_before', 'mean_absolute_error_after'), axis=1)
    df_INLP_mean["pretty_MSE-age_value"] = df_INLP_mean.apply(lambda row: _pretty_score(row, 'MSE_age_before', 'MSE_age_after'), axis=1)
    df_INLP_mean["pretty_MAE-age_value"] = df_INLP_mean.apply(lambda row: _pretty_score(row, 'MAE_age_before', 'MAE_age_after'), axis=1)
    df_INLP_mean["pretty_MSE-gender_value"] = df_INLP_mean.apply(lambda row: _pretty_score(row, 'MSE_gender_before', 'MSE_gender_after'), axis=1)
    df_INLP_mean["pretty_MAE-gender_value"] = df_INLP_mean.apply(lambda row: _pretty_score(row, 'MAE_gender_before', 'MAE_gender_after'), axis=1)
