...
def _pretty_model_name(row):
    pretty_name_mapping = {
        ...
        "SALBertForSequenceClassification": r"\textsc{SAL}",
        "SALAlbertForSequenceClassification": r"\textsc{SAL}",
        "SALRobertaForSequenceClassification": r"\textsc{SAL}",
        "SALGPT2ForSequenceClassification": r"\textsc{SAL}",
    }
...
def _parse_experiment_id(experiment_id):
    ...
    return model, model_name_or_path, bias_type, pre_assignment
...
if __name__ == "__main__":
    ...
    for result_file in tqdm(result_files, desc="Parsing GLUE results"):
        ...
        model, model_name_or_path, bias_type, pre_assignment = _parse_experiment_id(experiment_id)
    ...
    df = pd.merge(
        left=df,
        right=base_df[["experiment_id", "model", "pre_assignment"]],
        how="left",
        on="experiment_id",
    )
    ...
    with pd.option_context("max_colwidth", 1000):
        with open(
            f"{args.persistent_dir}/tables/glue_m-{args.model_type}.tex",
            "w",
        ) as f:
            f.write(
                df.to_latex(
