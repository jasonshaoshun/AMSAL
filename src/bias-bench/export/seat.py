...
def _pretty_model_name(row):
    pretty_name_mapping = {
        ...
        "SALBertModel": r"\textsc{SAL}",
        "SALAlbertModel": r"\textsc{SAL}",
        "SALRobertaModel": r"\textsc{SAL}",
        "SALGPT2Model": r"\textsc{SAL}",
    }
...
if __name__ == "__main__":
    ...
    # df["avg_absolute_effect_size"] = df_avg["avg_absolute_effect_size"]
    ...
    # Parse bias type from experiment ID.
    df["pre_assignment"] = df["experiment_id"].str.extract(
        r"seat_m-[A-Za-z0-9]+_c-[A-Za-z0-9-]+_*[A-Za-z-]*_p-([A-Za-z-]*)"
    )
    ...
    if file_exists(f"{args.persistent_dir}/results/seat/all.xlsx"):
        with pd.ExcelWriter(f"{args.persistent_dir}/results/seat/all.xlsx", mode='a') as writer:
            df.to_excel(
                writer,
                float_format="%.2f",
                index=False,
                columns=["model_type", "model", "pre_assignment", "bias_type"] + tests + ["avg_absolute_effect_size"],
                sheet_name=f"{args.model_type}_{args.bias_type}",
            )
    else:
        df.to_excel(
            f"{args.persistent_dir}/results/seat/all.xlsx",
            float_format="%.2f",
            index=False,
            columns=["model_type", "model", "pre_assignment", "bias_type"] + tests + ["avg_absolute_effect_size"],
            sheet_name=f"{args.model_type}_{args.bias_type}",
...
# columns=["model_type", "model", "pre_assignment", "bias_type"] 
# + test +  
# ["avg_absolute_effect_size", "pretty_avg_absolute_effect_size"]
