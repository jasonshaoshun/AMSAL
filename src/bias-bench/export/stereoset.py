...
if __name__ == "__main__":
    ...
    if file_exists(f"{args.persistent_dir}/results/stereoset/all.xlsx"):
        with pd.ExcelWriter(f"{args.persistent_dir}/results/stereoset/all.xlsx", mode='a') as writer:
            df.to_excel(
                writer,
                float_format="%.2f",
                columns=["model_type", "model", "pre_assignment", "bias_type", "stereotype_score", "language_model_score"],
                index=False,
                sheet_name=f"{args.model_type}_{args.bias_type}",
            )
    else:
        df.to_excel(
            f"{args.persistent_dir}/results/stereoset/all.xlsx",
            float_format="%.2f",
                columns=["model_type", "model", "pre_assignment", "bias_type", "stereotype_score", "language_model_score"],
            index=False,
            sheet_name=f"{args.model_type}_{args.bias_type}",
